# Copyright: IQGeo Limited 2010-2019

import os,sys,argparse,glob,re,warnings,json,code
...
class MywDbCommand( MywCommand ):
    ...
    arg_parser = argparse.ArgumentParser(prog=u"myw_db",formatter_class=MywArgparseHelpFormatter)
    arg_parser.add_argument("--version",action='version',version="%(prog)s "+MywCommand.version())
    arg_parser.epilog = "Utility for managing myWorld databases."

    arg_parser.add_argument("db_name",type=str, help="Name of PostgreSQL database") # ENH: Find a way to support unicode names
    arg_subparsers = arg_parser.add_subparsers(help='Operation to perform')
    ...
    def define_operation(arg_subparsers,operation,help):
        ...
        op_def = arg_subparsers.add_parser(operation,help=help,formatter_class=MywArgparseHelpFormatter)
    ...
    def add_standard_args(op_def):
        ...
        op_def.add_argument("--summary",  type=int, metavar='LEVEL', default=0, help="Summary output level")

        grp = op_def.add_argument_group("connect spec")
        grp.add_argument("--host",           type=unicode, help="Server on which Postgres is running (default: localhost)")
        grp.add_argument("--port",           type=int,     help="Port on which server listens (default: from pg_env)")
        grp.add_argument("--username", "-U", type=str,     help="Postgres user to connect as (default: from pg_env)") # ENH: Find a way to support unicode
        grp.add_argument("--password", "-P", type=str,     help="Password for Postgres user")                         # ENH: Find a way to support unicode
    ...
    def run_method(self,meth):
        ...
        self.db_server = MywDatabaseServer( db_type=db_type,
                                            host=self.args.host,
                                            port=self.args.port,
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                if self.args.verbosity > 10: 
                    raise
                msg_encoding = os.getenv('MYW_POSTGRES_MSG_ENCODING') or sys.getdefaultencoding()  # Work-around for encoding issue with sqlalchemy when connecting to database 
                raise MywError("Database error:",unicode(cond.message,msg_encoding))

        if self.args.summary:
            self.progress.print_statistics(self.args.summary)


    def replication_engine(self,db,db_type=None,remote_username=None,remote_password=None,ignore_sync_url=False):
        ##
        ...
        from myworldapp.core.server.replication.myw_replication_engine         import MywReplicationEngine
        from myworldapp.core.server.replication.myw_master_replication_engine  import MywMasterReplicationEngine
        from myworldapp.core.server.replication.myw_extract_replication_engine import MywExtractReplicationEngine

        # Determine type of engine to create
        if not db_type:
            db_type = MywReplicationEngine(db).databaseType()

        if   db_type == 'non-initialised': engine_exemplar = MywMasterReplicationEngine
        elif db_type == 'master':          engine_exemplar = MywMasterReplicationEngine
        elif db_type == 'extract':         engine_exemplar = MywExtractReplicationEngine
        ...
        if engine_exemplar != MywMasterReplicationEngine:
            if remote_username: extras['remote_username'] = remote_username
            if remote_password: extras['remote_password'] = remote_password
            if ignore_sync_url: extras['ignore_sync_url'] = ignore_sync_url

        # Create it
        return engine_exemplar(db,
                               db_type=db_type,
                               progress=self.progress,
                               **extras)
    ...
    def _dict_from_rec(self,rec,geom_as_str=False):
        ...
        for col in rec.__table__.columns:
            ...
            if val and hasattr(rec,'_descriptor'):
                field = rec._field(col.name)
                base_type = field.desc.type_desc.base

                if   base_type=='image': val = "image({}KB)".format( field.displayValue() ) # ENH: Encapsulate on model
                elif base_type=='file':  val = field.displayValue()
    ...
    op_def = define_operation(arg_subparsers,"create",help="Create new database")
    op_def.add_argument("--template",type=unicode,        help="Database to copy")
    op_def.add_argument("--overwrite",action='store_true',help="If database already exists, overwrite it")
    op_def.add_argument("--tilestore",type=unicode,       help="Used to adjust tilestore paths when template is existing database")
    ...
    op_def = define_operation(arg_subparsers,"install",help="Install myWorld tables")
    op_def.add_argument("module",        type=unicode, default=None,   help="Module to install")
    op_def.add_argument("--stop_before", type=int,     default=999999, help="Stop before this update")
    op_def.add_argument("--lang",        type=unicode,                 help="Language pack to take external names from (default: system language)")
    add_standard_args(op_def)
    ...
    def operation_install(self):
        ...
        if self.args.module == 'core': 
            upgrade_mgr.installCore(session,self.args.stop_before,self.args.lang)

        else:
            upgrade_mgr.installModule(self.args.module,session,self.args.stop_before,self.args.lang)
    ...
    op_def = define_operation(arg_subparsers,"upgrade",help="Upgrade database from an earlier release")
    op_def.add_argument("module",        type=str,                           help="Module to upgrade")
    op_def.add_argument("upgrade_id",    type=unicode, nargs="+",            help="Upgrade to run")
    op_def.add_argument("--stop_before", type=int,     default=999999,       help="Stop before this update")   
    op_def.add_argument("--lang",        type=str,                           help="Language pack to take external names from (default: system language)")
    op_def.add_argument("--test_run",    action="store_true", default=False, help="Just show show would be done")
    add_standard_args(op_def)
    ...
    def operation_upgrade(self):
        ...
        for upgrade_id in self.args.upgrade_id:
            ...
            with self.progress.operation('Running upgrade',upgrade_id):
                up = upgrade_engine(session,self.progress,self.args.lang)
                up.run(stop_before = self.args.stop_before,
                       dry_run     = self.args.test_run) 
    ...
    op_def = define_operation(arg_subparsers,"list",help="Show summary of database contents")
    op_def.add_argument("what", choices=['data','records','datasources',
                                         'features','fields','field_groups','queries','searches','filters',
                                         'enums','layers','layer_groups','private_layers','networks','applications',
                                         'roles','users','bookmarks','groups','settings','notifications','rights','versions','checkpoints',
                                         'table_sets','extracts','replicas','usage',
                                         'sequences','schema','schema_sql','system_tables','deltas','conflicts'], 
                        nargs="?", default='data',help="Type of information to list")
    op_def.add_argument("names",     type=unicode, nargs="?",default="*",                                        help="Item to list")
    op_def.add_argument("--full",    action="store_true",                                                        help="Long listing (shows additional details)")
    op_def.add_argument("--since",   type=unicode, metavar='CHECKPOINT',                                         help="Show changes since this checkpoint")
    op_def.add_argument("--delta",   type=unicode, default='',                                                   help="Show records in this database version")
    op_def.add_argument("--start",   type=unicode, metavar='DATE',                                               help="Start date for usage statistics")
    op_def.add_argument("--end",     type=unicode, metavar='DATE',                                               help="End date for usage statistics")
    op_def.add_argument("--by",      type=unicode, choices=['licence','application','layer','user','session','action',  
                                                            'month','week','day','hour'],default='licence',      help="Aggregation for usage statistics")
    op_def.add_argument("--limit",   type=int,                                                                   help="Maximum number of records to display")
    op_def.add_argument("--layout",  type=unicode, choices=MywTableFormatter.layouts, default='columns',         help="Format for ouput")
    add_standard_args(op_def)
    ...
    def operation_list(self):
        ...
        if   self.args.what == 'data':           self.list_counts(        db,self.args.names,self.args.layout,self.args.full,self.args.since,'data')
        elif self.args.what == 'system_tables':  self.list_counts(        db,self.args.names,self.args.layout,self.args.full,self.args.since,'myw') 
        elif self.args.what == 'records':        self.list_records(       db,self.args.names,self.args.layout,self.args.full,self.args.since,self.args.delta,self.args.limit)
        elif self.args.what == 'deltas':         self.list_deltas(        db,self.args.names,self.args.layout,self.args.full,self.args.since) 
        elif self.args.what == 'conflicts':      self.list_conflicts(     db,self.args.names,self.args.layout,self.args.full,self.args.delta) 
        elif self.args.what == 'datasources':    self.list_datasources(   db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'features':       self.list_features(      db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'fields':         self.list_fields(        db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'field_groups':   self.list_field_groups(  db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'queries':        self.list_queries(       db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'searches':       self.list_searches(      db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'filters':        self.list_filters(       db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'enums':          self.list_enums(         db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'layers':         self.list_layers(        db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'layer_groups':   self.list_layer_groups(  db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'private_layers': self.list_private_layers(db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'networks':       self.list_networks(      db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'applications':   self.list_applications(  db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'roles':          self.list_roles(         db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'users':          self.list_users(         db,self.args.names,self.args.layout,self.args.full)
        elif self.args.what == 'bookmarks':      self.list_bookmarks(     db,self.args.names,self.args.layout,self.args.full)
        elif self.args.what == 'groups':         self.list_groups(        db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'settings':       self.list_settings(      db,self.args.names,self.args.layout,self.args.full,self.args.since)
        elif self.args.what == 'notifications':  self.list_notifications( db,self.args.names,self.args.layout,self.args.full)
        elif self.args.what == 'rights':         self.list_rights(        db,self.args.names,self.args.layout,self.args.full)
        elif self.args.what == 'versions':       self.list_versions(      db,self.args.names,self.args.layout,self.args.full) 
        elif self.args.what == 'checkpoints':    self.list_checkpoints(   db,self.args.names,self.args.layout,self.args.full) 
        elif self.args.what == 'table_sets':     self.list_table_sets(    db,self.args.names,self.args.layout,self.args.full,self.args.since) 
        elif self.args.what == 'extracts':       self.list_extracts(      db,self.args.names,self.args.layout,self.args.full) 
        elif self.args.what == 'replicas':       self.list_replicas(      db,self.args.names,self.args.layout,self.args.full) 
        elif self.args.what == 'sequences':      self.list_sequences(     db,self.args.names,self.args.layout,self.args.full) 
        elif self.args.what == 'schema':         self.list_schema(        db,self.args.names,self.args.layout,self.args.full) 
        elif self.args.what == 'schema_sql':     self.list_schema_sql(    db,self.args.names,self.args.layout,self.args.full) 
        elif self.args.what == 'usage':          self.list_usage(         db,self.args.names,self.args.layout,self.args.by,self.args.start,self.args.end,self.args.full)
        else:
    ...
    def list_counts(self,db,name_spec,layout,full,since,default_schema):
        ...
        if schema=='myw':
            ...
            table_names = db.dd.featureTypes('myworld',name_spec,versioned_only=(schema!='data'),sort=True,warn_if_no_match=True)

        # List them
        if since == None: self.list_counts_full(db,schema,table_names,layout,full)
        else:             self.list_counts_since(db,schema,table_names,layout,since)
    ...
    def list_counts_since(self,db,schema,table_names,layout,checkpoint):
        ...
        for tab_name in table_names:
            ...
            if   schema=='myw':  changes = db.configChanges(tab_name,since_version)
            elif schema=='data': changes = db.featureChanges(tab_name,since_version)
            else:                changes = db.deltaChanges(tab_name,since_version,schema)
            ...
            if n_recs > 0:
                ...
                row = { 'table': tab_name,
                        'count':   n_recs,
                        'inserts': changes.values().count('insert'),
                        'updates': changes.values().count('update'),
                        'deletes': changes.values().count('delete') }
    ...
    def list_records(self,db,name_spec,layout,full,since,delta,limit):
        ...
        feature_types = db.dd.featureTypes('myworld',name_spec,
        ...
        base_cols = []
        if delta:             base_cols.append('delta')
        if delta or since:    base_cols.append('change')
        ...
        if full:
            for feature_type in feature_types:
                ...
                field_names = feature_rec.fieldRecs(stored_only=True).keys()
    ...
    def _get_record_rows(self,db,feature_types,delta='',since_version=None,limit=None,geom_as_str=False):
        ##
        ...
        for feature_type in feature_types:
            ...
            # Case: Delta only
            #DELTAS: Remove this duplication
            if delta:

                for rec in db._deltaRecsFor(feature_type,delta).order_by('myw_delta'):

                    row = { 'delta': rec.myw_delta,
            ...
            # Case: Changes only
                ...
                for key_val,change in db.featureChanges(feature_type,since_version).items():

                    row = { 'record': "{}({})".format(feature_type,key_val),
                            'change': change }

                    if change != 'delete':
                        rec = db.session.query(model).get(key_val)
                        row.update( self._dict_from_rec(rec,geom_as_str) )
                        row['myw_title']             = rec._title()            # ENH: Extend table formatter to support computed columns
                        row['myw_short_description'] = rec._shortDescription() 
    ...
    def _list_deltas_since(self, db, name_spec, layout, since, full):
        ...
        for feature_type in db.dd.featureTypes('myworld',versioned_only=True,sort=True):
            ...
            for (delta,id),change_type in changes.items():
                stat = stats.get(delta)

                if not stat:
                    stat = stats[delta] = {'delta': delta,
                                           'insert': 0,
                                           'update': 0,
                                           'delete': 0,
                                           'count' : 0}
    ...
    def _list_deltas_full(self, db, name_spec, layout, full):
        ...
        for delta in sorted(stats):
            ...
            for ft_name,ft_stat in stat.items():
                row[ft_name] = ft_stat

                for change_type,n_recs in ft_stat.items():
        ...
        for row in rows:
            for ft_name in feature_types:
                ...
                if ft_stat:
                    row[ft_name] = "{}:{}:{}".format( ft_stat['insert'],ft_stat['update'],ft_stat['delete'] )
    ...
    def list_features(self, db, name_spec, layout, full, since):
        ...
        def rowFrom(rec):
            ...
            row['filter_fields'] = ",".join( rec.filter_map().values() )
        ...
        if since == None:
            ...
            for rec in db.dd.featureTypeRecs(datasource_spec,feature_type_spec,sort=True,warn_if_no_match=True):
                row = rowFrom(rec)
                rows.append(row)
            ...
            for (feature_name,change) in db.configChanges('dd_feature',since_version,name_spec).items():
    ...
    def list_fields(self,db,name_spec,layout,full,since):
        ...
        rows = []
        for feature_rec in db.dd.featureTypeRecs(datasource_spec,feature_type_spec,sort=True,warn_if_no_match=True):
            ...
            for field_rec in feature_rec.dd_field_recs:
                ...
                field_rec.full_name = u"{}.{}".format(field_rec.table_name,field_rec.internal_name)

                if field_rec.min_value != None:
                    field_rec.range = u"{}:{}".format(field_rec.min_value,field_rec.max_value)
                else:
        ...
        if full:
            cols += ['enum','generator','unit','display_unit','unit_scale','display_format','range','mandatory','default','indexed']
        tab_fmtr = MywTableFormatter(*cols)
        ...
        for feature_rec in db.dd.featureTypeRecs(datasource_spec,feature_type_spec,sort=True,warn_if_no_match=True):
            for group_rec in feature_rec.field_group_recs:
                ...
                if not full:
                    ...
                    row['fields'] = len( group_rec.items() )
                    rows.append(row)
                else:
                    for item_rec in group_rec.items():
                        row = self._dict_from_rec(group_rec)
                        row['field_name'] = item_rec.field_name
                        rows.append(row)

        # Build table formatter
        cols = [ ['feature_name','feature'],
                 ['display_name','group'],
    ...
    def list_filters(self, db, name_spec, layout, full, since):
        ...
        # Build table formatter
        tab_fmtr = MywTableFormatter(['feature_name','feature'],
                                     ['name','filter'],
                                     'value')
    ...
    def _list_enum_counts(self, db, name_spec, layout, since):
        ...
        if since == None:
            ...
            for enum_name in db.dd.enumeratorNames(name_spec,sort=True,warn_if_no_match=True):
                ...
                row = {'enum':   enum_name,
                       'count': len(enum_rec.value_recs.all()) }
            ...
            for enum_name,change in db.configChanges('dd_enum',since_version,name_spec).items():
    ...
    def list_applications(self, db, name_spec, layout, full, since):
        ...
        if since == None:
            ...
            # Show all records
            for app_name in db.config_manager.applicationNames(name_spec,sort=True,warn_if_no_match=True):
            ...
            for app_name,change in db.configChanges('application',since_version,name_spec).items():
                row = { 'change':      change, 
                        'name':        app_name }
                if change != 'delete':
                    rec = db.config_manager.applicationRec(app_name)
    ...
    def list_roles(self, db, name_spec, layout, full, since):
        ...
        if since is None:
            ...
            # Show all records
            for layer_name in db.config_manager.roleNames(name_spec,sort=True,warn_if_no_match=True):
            ...
            for role_name,change in db.configChanges('role',since_version,name_spec).items():
                row = { 'change':      change, 
                        'name':        role_name }
                if change != 'delete':
    ...
    def list_datasources(self, db, name_spec, layout, full, since):
        ...
        if since == None:
            ...
            for (datasource_name,change) in db.configChanges('datasource',since_version,name_spec).items():
                row = { 'change': change, 
    ...
    def list_layers(self, db, name_spec, layout, full, since):
        ...
        if since == None:
            ...
            for (layer_name,change) in db.configChanges('layer',since_version,name_spec).items():
                row = { 'change': change, 
                        'name': layer_name}
        ...
        for layer_def in rows:
            ...
            layer_def['scales'] = "{}:{}".format( layer_def.get('min_scale',"-"),
                                                  layer_def.get('max_scale',"-") )

        # Display it
        cols = base_cols + ['category','datasource','type','code']
        if full:
            cols += ['scales',
                     'description', 
                     ['spec','spec',100],
    ...
    def list_layer_groups(self, db, name_spec, layout, full, since):
        ...
        if since == None:
            ...
            for (name,change) in db.configChanges('layer_group',since_version,name_spec).items():
                row = { 'change': change, 
                        'name': name}
    ...
    def list_private_layers(self, db, name_spec, layout, full, since):
        ...
        if since == None:
            ...
            for (rec_id,change) in db.configChanges('private_layer',since_version,name_spec).items():
    ...
    def list_networks(self, db, name_spec, layout, full, since):
        ...
        if since == None:
            ...
            for (name,change) in db.configChanges('network',since_version,name_spec).items():
                row = { 'change': change, 
                        'name': name}
    ...
    def list_groups(self, db, name_spec, layout, full, since):
        ...
        if since == None:
            ...
            for (rec_id,change) in db.configChanges('group',since_version,name_spec).items():
                row = { 'change': change, 
                        'group': rec_id }
                ...
                if change != 'delete':
                    ...
                    row['members']     = u":".join( members.keys() )
        ...
        if since == None:
            ...
            for name in db.settings(name_spec):
                rows.append( { 'setting':  name, 
                               'value':    db.setting(name) } )

        else:
            base_cols = ['change']
            since_version = db.dataVersionFor(since)

            for (name,change) in db.configChanges('setting',since_version,name_spec).items():
                row = { 'change': change, 
                        'setting': name}
    ...
    def list_table_sets(self, db, name_spec, layout, full, since):
        ...
        if since == None:
            ...
            for (name,change) in db.configChanges('table_set',since_version,name_spec).items():
                row = { 'change': change, 
                        'name': name}
    ...
    def list_extracts(self, db, name_spec, layout, full):
        ...
        # Display it
        cols = [ 'name', 'region', 'table_set', 'last_export' ]
        if full:
            cols += ['hours_ago']
    ...
    def list_sequences(self, db, name_spec, layout, full):
        ...
        for (schema,table_name) in self.tableSpecsMatching(db,name_spec):
            ...
            for column_name,column_desc in table_desc.columns.items():
    ...
    def list_schema(self, db, name_spec, layout, full):
        ...
        for (schema,table_name) in self.tableSpecsMatching(db,name_spec):
            ...
            # Show fields
            rows = table_desc.columns.values()
            tab_fmtr = MywTableFormatter( ['table_name','table'],
                                          ['name','field'],
                                          ['db_type','type'],
                                          'key',
                                          'default',
            ...
            # Show triggers
            rows = []
            for trigger_name,defn in db_driver.sortedTriggersFor(schema,table_name).items():
                for line in defn['body'].splitlines(): # ENH: Support multiline values in MywTableFormatter
    ...
    def list_usage(self, db, name_spec, layout, by, start, end, full):
        ...
        if by=='session':
            ...
            rows = []
            for licence,users in stats_manager.usageByLicence(name_spec,start,end).items():
            ...
            for row in stats_manager.usageProfile(by,start=start,end=end):
                ...
                row['end']       = datetime.strftime( row['period_end'] - timedelta(seconds=1), datetime_format)
    ...
    op_def = define_operation(arg_subparsers,"load",help="Load enumerators, feature definitions and data")
    op_def.add_argument("files",             type=unicode, nargs="+",                             help="Data or configuration file to load (can be wildcard)")
    op_def.add_argument("--as",              type=unicode, dest="load_as", metavar='NAME',        help="New name to be used when loading data")
    op_def.add_argument("--encoding",        type=unicode, default='utf_8',                       help="File encoding")
    op_def.add_argument("--reload",          action='store_true',                                 help="Drop existing data before loading")
    op_def.add_argument("--update",          action='store_true',                                 help="Update existing data with changes in loaded file")
    op_def.add_argument("--force",           action='store_true',                                 help="Reload even if in use")
    op_def.add_argument("--date_format",     type=unicode, metavar='FORMAT',default='YYYY-MM-DD', help="Date format in file")
    op_def.add_argument("--time_format",     type=unicode, metavar='FORMAT',default='HH:MI:SS.FF',help="Time format in file")
    op_def.add_argument("--coord_system",    type=unicode, metavar='NAME',                        help="Coordinate system of file (epsg name)")
    op_def.add_argument("--autocreate",      action='store_true',                                 help="Create feature definitions based on data shape (if necessary)")
    op_def.add_argument("--key",             type=unicode,metavar='FIELD',                        help="Field to use as key (when autcreating)")
    op_def.add_argument("--build_geoms",     action='store_true',                                 help="Use heuristics to build geometries from record attributes")
    op_def.add_argument("--delta",           type=unicode, default='',                            help="Load data into this database version")
    op_def.add_argument("--update_sequence", action='store_true',                                 help="Update feature ID generator high-water-mark")
    op_def.add_argument("--direct",          action='store_true',                                 help="Use native database feature loader (for speed)")
    add_standard_args(op_def)
    ...
    def operation_load(self):
        ...
        for file_spec in self.args.files:
            ...
            file_spec = unicode(file_spec)  # Forces glob to return unicode names # ENH: Use os_engine
    ...
    def load_file(self,db,file_path):
        ...
        try:
            (n_processed,msg) = db.data_loader.loadFile(file_path, 
            ...
            self.progress('error',unicode(cond) )
    ...
    op_def = define_operation(arg_subparsers,"import",help="Import information from an external datasource")
    op_def.add_argument("what",         type=unicode, choices=['features'], help="Type of information to import")
    op_def.add_argument("feature_type", type=unicode,                       help="Feature type to import data for (can be wildcard)")
    add_standard_args(op_def)
    ...
    def import_features(self,db,ds_rec,ds_engine,feature_type_spec):
        ...
        for feature_type in ds_engine.feature_types(feature_type_spec):
            ...
            with self.progress.operation("Importing feature definition:",ds_rec.name+'/'+feature_type):
    ...
    op_def = define_operation(arg_subparsers,"configure",help="Configure feature properties")
    op_def.add_argument("feature_type",        type=unicode,                           help="Feature type to modify (can be wildcard)")
    op_def.add_argument("--external_name",     type=unicode,                           help="String used to identify feature type in GUI")
    op_def.add_argument("--title",             type=unicode,                           help="Expression used to build myw_title field")
    op_def.add_argument("--short_description", type=unicode,                           help="Expression used to build myw_short_description field")
    op_def.add_argument("--track_changes",     type=unicode, choices=['true','false'], help="Determines if updates are tracked")
    op_def.add_argument("--versioned",         type=unicode, choices=['true','false'], help="Determines if deltas are supported")
    op_def.add_argument("--editable",          type=unicode, choices=['true','false'], help="Determines if object can be modified")
    add_standard_args(op_def)
    ...
    def operation_configure(self):
        ...
        props = {}
        if self.args.external_name     != None: props['external_name']     = self.args.external_name 
        if self.args.title             != None: props['title']             = self.args.title
        if self.args.short_description != None: props['short_description'] = self.args.short_description
        if self.args.track_changes     != None: props['track_changes']     = self.args.track_changes == 'true'
        if self.args.versioned         != None: props['versioned']         = self.args.versioned == 'true'
        if self.args.editable          != None: props['editable']          = self.args.editable == 'true'
    ...
    op_def = define_operation(arg_subparsers,"add",help="Add a search or query")
    op_def.add_argument("what",type=unicode, choices=['search','query','application_layer','notification'], help="Type of item to add")
    op_def.add_argument("args",type=unicode, nargs="*",                                                     help="Definition of item (depends on type)")
    add_standard_args(op_def)

    add_op_defs = {}

    add_op_defs['search'] = op_def = argparse.ArgumentParser(prog='add search',formatter_class=MywArgparseHelpFormatter)
    op_def.add_argument("feature_type", type=unicode,                        help="Feature type to add item too (can be wildcard)")
    op_def.add_argument("value",        type=unicode,                        help="Matched value (myWorld expression)")
    op_def.add_argument("description",  type=unicode,                        help="Dsiplay value (myWorld expression)")

    add_op_defs['query'] = op_def = argparse.ArgumentParser(prog='add query',formatter_class=MywArgparseHelpFormatter)
    op_def.add_argument("feature_type", type=unicode,                        help="Feature type to add item too (can be wildcard)")
    op_def.add_argument("value",        type=unicode,                        help="Matched value (myWorld expression)")
    op_def.add_argument("description",  type=unicode,                        help="Display value (myWorld expression)")
    op_def.add_argument("filter",       type=unicode, nargs="?", default="", help="Filter (myWorld filter)")

    add_op_defs['application_layer'] = op_def = argparse.ArgumentParser(prog='add application_layer',formatter_class=MywArgparseHelpFormatter)
    op_def.add_argument("application", type=unicode,                         help="Application to add layer too (can be wildcard)")
    op_def.add_argument("layer",       type=unicode,                         help="Layer to add (can be wildcard)")

    add_op_defs['notification'] = op_def = argparse.ArgumentParser(prog='add notification',formatter_class=MywArgparseHelpFormatter)
    op_def.add_argument("type",    type=unicode, choices=['alert','info','tip'],                              help="Message type")
    op_def.add_argument("subject", type=unicode,                                                              help="Message subject")
    op_def.add_argument("details", type=unicode,  nargs="?", default='',                                      help="Message body")
    op_def.add_argument("users",   type=unicode, choices=['all','online','native'], nargs="?", default='all', help="Target user group")
    ...
    def operation_add(self):
        ...
        # Do operation
        if   self.args.what == "search":             self.add_search(db,op_args.feature_type,op_args.value,op_args.description)
        elif self.args.what == "query":              self.add_query(db,op_args.feature_type,op_args.value,op_args.description,op_args.filter)
        elif self.args.what == "application_layer":  self.add_application_layer(db,op_args.application,op_args.layer)
        elif self.args.what == "notification":       self.add_notification(db,op_args.type,op_args.subject,op_args.details,op_args.users)

        db.commit()


    def add_search(self,db,name_spec,value,description):
        ##
        ...
        for feature_rec in db.dd.featureTypeRecs(datasource_spec,feature_type_spec,sort=True,warn_if_no_match=True):
            ...
            with self.progress.operation("Adding search to",feature_rec):
                feature_desc = db.dd.featureTypeDescriptor(feature_rec)
                feature_desc.addSearch(value,description)
                db.dd.alterFeatureType(feature_rec,feature_desc)


    def add_query(self,db,name_spec,value,description,filter):
        ##
        ...
        for feature_rec in db.dd.featureTypeRecs(datasource_spec,feature_type_spec,sort=True,warn_if_no_match=True):
            ...
            with self.progress.operation("Adding query to",feature_rec):
                feature_desc = db.dd.featureTypeDescriptor(feature_rec)
                feature_desc.addQuery(value,description,filter)
                db.dd.alterFeatureType(feature_rec,feature_desc)


    def add_application_layer(self,db,application_name_spec,layer_name_spec):
        ##
        ## Add layer to application (if not already present)
        ##

        application_names = db.config_manager.applicationNames(application_name_spec,include_config=False,sort=True,warn_if_no_match=True)
        layer_names       = db.config_manager.layerNames(layer_name_spec,sort=True,warn_if_no_match=True)

        for application_name in application_names:
            application_rec = db.config_manager.applicationRec(application_name)

            app_layer_names = application_rec.layers()

            for layer_name in layer_names:
                if not layer_name in app_layer_names:
                    self.progress(1,"Adding layer to application:",application_name,":",layer_name)
                    app_layer_names.append(layer_name)

            application_rec.set_layers(app_layer_names)
    ...
    op_def = define_operation(arg_subparsers,"dump",help="Output data or metadata to file")
    op_def.add_argument("output_dir",          type=unicode,                                               help="Directory to create files in")
    op_def.add_argument("what",                type=unicode, choices=['data','deltas','features','enums',
                                                                      'layers','layer_groups','private_layers','networks','datasources',
                                                                      'applications','roles','users','groups','table_sets','settings'], help="Type of information to dump")
    op_def.add_argument("names",               type=unicode, nargs='?',  default='*',                      help="Name of item(s) to dump")
    op_def.add_argument("--since",             type=unicode, metavar='CHECKPOINT',                         help="Output only those items changed since this checkpoint")
    op_def.add_argument("--delta",             type=unicode, default='',                                   help="Show records in this database version")
    op_def.add_argument("--encoding",          type=unicode, default='utf_8',                              help="File encoding")
    op_def.add_argument("--format",            type=unicode, default='csv(ewkb)',                          help="Format for 'data' files, with optional modifiers")
    op_def.add_argument("--fields",            type=unicode, nargs='+',  metavar="NAME",                   help="Fields to include in data")
    op_def.add_argument("--exclude_fields",    type=unicode, nargs='+',  metavar="NAME",                   help="Fields to exclude from data")
    op_def.add_argument("--area",              type=unicode,                                               help="Geographic region to dump (lon1,lat1):(lon2,lat2)" )
    op_def.add_argument("--coord_system",      type=unicode, metavar='NAME', default='epsg:4326',   help="Coordinate system to output data in")
    op_def.add_argument("--date_format",       type=unicode, metavar='FORMAT',default='YYYY-MM-DD',        help="Output format for date values")
    op_def.add_argument("--time_format",       type=unicode, metavar='FORMAT',default='HH:MI:SS.FF',       help="Output format for time values")
    op_def.add_argument("--records_per_file",  type=int, default=10000, metavar='N',                       help="Start new output file after N records")
    op_def.add_argument("--checkpoint",        type=unicode, metavar='NAME',                               help="Checkpoint to create in database (atomic)")
    add_standard_args(op_def)
    ...
    def operation_dump(self):
        ...
        if self.args.what == 'data':             self.dump_data(output_dir,db,self.args.names,self.args.encoding,self.args.format,
                                                                self.args.date_format,self.args.time_format,coord_sys,
                                                                since_version,self.args.delta,region_geom,self.args.records_per_file,
                                                                self.args.fields, self.args.exclude_fields)

        elif self.args.what == 'deltas':         self.dump_deltas(output_dir,db,self.args.names,region_geom,self.args.encoding,since_version)
        elif self.args.what == 'features':       self.dump_feature_types(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'enums':          self.dump_enums(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'layers':         self.dump_layers(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'layer_groups':   self.dump_layer_groups(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'private_layers': self.dump_private_layers(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'networks':       self.dump_networks(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'datasources':    self.dump_datasources(output_dir,db,self.args.names,self.args.encoding,since_version)  
        elif self.args.what == 'applications':   self.dump_applications(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'roles':          self.dump_roles(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'users':          self.dump_users(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'groups':         self.dump_groups(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'table_sets':     self.dump_table_sets(output_dir,db,self.args.names,self.args.encoding,since_version)
        elif self.args.what == 'settings':       self.dump_settings(output_dir,db,self.args.names,self.args.encoding,since_version)
    ...
    def dump_data(self,output_dir,db,feature_type_spec,file_encoding,file_format,date_format,time_format,coord_sys,
                  since_version,delta,region_geom,max_recs_per_file,fields,exclude_fields):
        ##
        ...
        file_options = {} 
        if 'compact' in options: file_options['compact'] = True
        if 'wkb'     in options: file_options['geom_encoding'] = 'wkb'
        if 'ewkb'    in options: file_options['geom_encoding'] = 'ewkb'
        if 'wkt'     in options: file_options['geom_encoding'] = 'wkt'
        if 'ewkt'    in options: file_options['geom_encoding'] = 'ewkt'
        ...
        for feature_type in db.dd.featureTypes('myworld',feature_type_spec,versioned_only=versioned_only,sort=True,warn_if_no_match=True):
            pred = self.geomFilterFor(db,feature_type,region_geom)

            # Case: Full dump
            if since_version == None and not delta:  #DELTAS: Support full dump from delta?

                with self.progress.operation("Feature",feature_type,"...") as stats:
                    stats['records'] = db.data_loader.dumpFeatures( output_dir,feature_type,
                                                                    pred             =pred,
            ...
            # Case: Incremental dump
                ...
                if delta:                      #DELTAS: handle --since and --delta set
                ...
                if changes:
                    with self.progress.operation("Feature",feature_type,"...") as stats:
                        stats['records'] = db.data_loader.dumpFeatureChanges( output_dir,feature_type,changes,
                                                                              delta            =delta,
                                                                              pred             =pred,
                                                                              file_encoding    =file_encoding,
                                                                              file_format      =file_format,
    ...
    def dump_deltas(self,output_dir,db,name_spec,region_geom,file_encoding,since_version):
        ...
        for feature_type in db.dd.featureTypes('myworld',versioned_only=True,sort=True):
            ...
            if since_version == None:
                n_recs = db.data_loader.dumpDeltas(output_dir,feature_type,name_spec,pred=pred,file_encoding=file_encoding)
                self.progress(2,"Wrote",n_recs,"records")
                
            # Case: Incremental dump
            else:
                delta_changes = db.deltaChanges(feature_type,since_version,schema='delta') #DELTAS: Include name_spec
                base_changes  = db.deltaChanges(feature_type,since_version,schema='base')  #DELTAS: Include name_spec

                if delta_changes or base_changes:
                    n_recs = db.data_loader.dumpDeltaChanges(output_dir,feature_type,delta_changes,base_changes,pred=pred,file_encoding=file_encoding)
                    self.progress(2,"Wrote",n_recs,"records")
    ...
    def dump_feature_types(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for feature_rec in db.dd.featureTypeRecs(datasource_spec,feature_type_spec,sort=True,warn_if_no_match=True):
                db.data_loader.dumpFeatureType(output_dir,feature_rec,    
                                               file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental dump
            ...
            n_done = db.data_loader.dumpFeatureTypeChanges(output_dir,changes,
                                                           file_encoding=file_encoding)
    ...
    def dump_enums(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for enum_name in db.dd.enumeratorNames(name_spec,sort=True,warn_if_no_match=True):
                db.data_loader.dumpEnumerator(output_dir,enum_name,
                                              file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental dump
            ...
            n_done = db.data_loader.dumpEnumeratorChanges(output_dir,changes,
                                                          file_encoding=file_encoding)
    ...
    def dump_layers(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for layer_name in db.config_manager.layerNames(name_spec,sort=True,warn_if_no_match=True):
                db.data_loader.dumpLayerDefinition(output_dir,layer_name,
                                                   file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental dump
            ...
            n_done = db.data_loader.dumpLayerChanges(output_dir,changes,
                                                     file_encoding=file_encoding)
    ...
    def dump_layer_groups(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for layer_name in db.config_manager.layerGroupNames(name_spec,sort=True,warn_if_no_match=True):
                db.data_loader.dumpLayerGroupDefinition(output_dir,layer_name,
                                                        file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental dump
            ...
            n_done = db.data_loader.dumpLayerGroupChanges(output_dir,changes,
                                                          file_encoding=file_encoding)
    ...
    def dump_private_layers(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for rec in db.config_manager.privateLayerRecs(name_spec,sort=True, warn_if_no_match=True):
                db.data_loader.dumpPrivateLayer(output_dir, rec,
                                                file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental Dump
            ...
            n_done = db.data_loader.dumpPrivateLayerChanges(output_dir,changes,
                                                            file_encoding=file_encoding)
    ...
    def dump_networks(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for network_name in db.config_manager.networkNames(name_spec,sort=True,warn_if_no_match=True):
                db.data_loader.dumpNetworkDefinition(output_dir,network_name,
                                                     file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental dump
            ...
            n_done = db.data_loader.dumpNetworkChanges(output_dir,changes,
                                                       file_encoding=file_encoding)
    ...
    def dump_datasources(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for datasource_name in db.dd.datasourceNames(name_spec,sort=True,warn_if_no_match=True):
                db.data_loader.dumpDatasourceDefinition(output_dir,datasource_name,
                                                        file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental dump
            ...
            n_done = db.data_loader.dumpDatasourceChanges(output_dir,changes,
                                                          file_encoding=file_encoding)
    ...
    def dump_applications(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for application_name in db.config_manager.applicationNames(name_spec,sort=True, warn_if_no_match=True):
                db.data_loader.dumpApplication(output_dir,application_name,
                                               file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental dump
            ...
            n_done = db.data_loader.dumpApplicationChanges(output_dir,changes,
                                                           file_encoding=file_encoding)
    ...
    def dump_roles(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            for role in db.config_manager.roleNames(name_spec,sort=True, warn_if_no_match=True):
                db.data_loader.dumpRole(output_dir, role, 
                                        file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental Dump
            ...
            n_done = db.data_loader.dumpRoleChanges(output_dir,changes,
                                                    file_encoding=file_encoding)
    ...
    def dump_users(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            for user in db.config_manager.userNames(name_spec,sort=True, warn_if_no_match=True):
                db.data_loader.dumpUser(output_dir, user, 
                                        file_encoding=file_encoding)
                n_done += 1
    ...
    def dump_groups(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for group_rec in db.config_manager.groupRecs(name_spec,sort=True, warn_if_no_match=True):
                db.data_loader.dumpGroup(output_dir, group_rec,
                                         file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental Dump
            ...
            n_done = db.data_loader.dumpGroupChanges(output_dir,changes,
                                                     file_encoding=file_encoding)
    ...
    def dump_table_sets(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        if since_version == None:
            ...
            n_done = 0
            for name in db.config_manager.tableSetNames(name_spec,sort=True, warn_if_no_match=True):
                db.data_loader.dumpTableSet(output_dir, name, 
                                            file_encoding=file_encoding)
                n_done += 1
        ...
        # Case: Incremental Dump
            ...
            n_done = db.data_loader.dumpTableSetChanges(output_dir,changes,
                                                        file_encoding=file_encoding)
    ...
    def dump_settings(self,output_dir,db,name_spec,file_encoding,since_version):
        ...
        # Case: Full dump
        if since_version == None:
            n_done = db.data_loader.dumpSettings(output_dir,name_spec,
                                                 file_encoding=file_encoding)
        ...
        # Case: Incremental Dump
            ...
            n_done = db.data_loader.dumpSettingChanges(output_dir,changes,
                                                       file_encoding=file_encoding)
    ...
    op_def = define_operation(arg_subparsers,"drop",help="Delete data or meta-data")
    op_def.add_argument("what", type=unicode, choices=['data','features','searches','queries','enums',
                                                       'layers','layer_groups','private_layers','networks','datasources',
                                                       'applications','roles','users','groups','settings','notifications',
                                                       'checkpoints','table_sets','extracts','replicas','deltas'], help="Type of object to drop")
    op_def.add_argument("name",    type=unicode,             help="Name of item(s) to drop (can be wildcard)")
    op_def.add_argument("--delta", type=unicode, default='', help="Drop data records from this database version")
    ...
    def operation_drop(self):
        ...
        if   self.args.what == 'data':           self.drop_data(db,self.args.name,self.args.delta)
        elif self.args.what == 'features':       self.drop_features(db,self.args.name,self.args.force)
        elif self.args.what == 'searches':       self.drop_searches(db,self.args.name)
        elif self.args.what == 'queries':        self.drop_queries(db,self.args.name)
        elif self.args.what == 'enums':          self.drop_enums(db,self.args.name,self.args.force)
        elif self.args.what == 'layers':         self.drop_layers(db,self.args.name,self.args.force)
        elif self.args.what == 'layer_groups':   self.drop_layer_groups(db,self.args.name)
        elif self.args.what == 'private_layers': self.drop_private_layers(db,self.args.name)
        elif self.args.what == 'networks':       self.drop_networks(db,self.args.name)
        elif self.args.what == 'datasources':    self.drop_datasources(db,self.args.name,self.args.force)
        elif self.args.what == 'applications':   self.drop_applications(db, self.args.name)
        elif self.args.what == 'roles':          self.drop_roles(db, self.args.name)
        elif self.args.what == 'users':          self.drop_users(db, self.args.name)
        elif self.args.what == 'groups':         self.drop_groups(db, self.args.name)
        elif self.args.what == 'settings':       self.drop_settings(db, self.args.name)
        elif self.args.what == 'notifications':  self.drop_notifications(db, self.args.name)
        elif self.args.what == 'checkpoints':    self.drop_checkpoints(db,self.args.name)
        elif self.args.what == 'table_sets':     self.drop_table_sets(db,self.args.name,self.args.force)
        elif self.args.what == 'extracts':       self.drop_extracts(db,self.args.name,self.args.force)
        elif self.args.what == 'replicas':       self.drop_replicas(db,self.args.name,self.args.force)
        elif self.args.what == 'deltas':         self.drop_deltas(db,self.args.name)
        else:
            raise Exception("Bad choice: "+self.args.what) # Should never happen
    ...
    def drop_data(self,db,feature_type_spec,delta):
        ...
        ## Drop feature data
        ##
        #DELTAS: Support wildcarded delta?
        
        if delta:
            db_view = db.view(delta)

            for feature_type in db.dd.featureTypes('myworld',feature_type_spec,versioned_only=True,sort=True,warn_if_no_match=True):
                with self.progress.operation("Delta",delta,":","Dropping data for:",feature_type):
                    db_view.table(feature_type).truncate()
                    
        else:
            for feature_type in db.dd.featureTypes('myworld',feature_type_spec,sort=True,warn_if_no_match=True):
    ...
    def drop_features(self,db,name_spec,force):
        ...
        for feature_rec in db.dd.featureTypeRecs(datasource_spec,feature_type_spec,sort=True,warn_if_no_match=True):
            ...
            with self.progress.operation("Dropping feature type:",feature_rec):
                ...
                try: 
                    if feature_rec.datasource_name=='myworld' and not db.dd.featureTableIsEmpty(feature_rec.feature_name):
                        if not force:
                    ...
                    self.progress('warning', unicode(cond) )
    ...
    op_def = define_operation(arg_subparsers,"checkpoint",help="Create a named version")
    op_def.add_argument("name",         type=unicode,        help="Checkpoint name")
    op_def.add_argument("--at",         type=unicode,        help="Version to create checkpoint at (default: current version)")
    op_def.add_argument("--reposition", action="store_true", help="If checkpoint already exists, reposition it")
    add_standard_args(op_def)
    ...
    op_def = define_operation(arg_subparsers,"maintain",help="Maintain statistics and internal tables")
    add_standard_args(op_def)
    op_def.add_argument("what", choices=['statistics','disk','transaction_logs',
                                         'features','triggers','geom_indexes','searches',
                                         'notifications','replicas','usage_stats'       ], help="Aspect to maintain")

    op_def.add_argument("names", type=unicode, nargs="?", default="*",   help="Item(s) to perform operation on (can be wildcard)")
    op_def.add_argument("--before", metavar='DAYS_OR_DATE', default="30", help="Minimum age to keep (notifications and usage_stats only)")
    ...
    def operation_maintain(self):
        ...
        if   self.args.what == 'statistics':       self.maintain_statistics(db)
        elif self.args.what == 'disk':             self.maintain_disk(db,self.args.names)
        elif self.args.what == 'transaction_logs': self.maintain_transaction_logs(db)
        elif self.args.what == 'features':         self.maintain_objects(db,self.args.names)
        elif self.args.what == 'triggers':         self.maintain_triggers(db,self.args.names)
        elif self.args.what == 'geom_indexes':     self.maintain_geom_indexes(db,self.args.names)
        elif self.args.what == 'searches':         self.maintain_searches(db,self.args.names)
        elif self.args.what == 'notifications':    self.maintain_notifications(db,self.args.names,before_date)
        elif self.args.what == 'replicas':         self.maintain_replicas(db) # ENH: Use names and age
        elif self.args.what == 'usage_stats':      self.maintain_usage_stats(db,self.args.names,before_date)
        else:
            raise Exception('Bad choice: '+self.args.what)  # Should never happen
    ...
    def maintain_objects(self,db,feature_type):
        ...
        for feature_rec in db.dd.featureTypeRecs('myworld',feature_type,sort=True,warn_if_no_match=True):
    ...
    def maintain_notifications(self,db,subject_spec,min_date_to_keep):
        ...
        with self.progress.operation("Pruning notifications created before:",min_date_to_keep.date()):
    ...
    op_def = define_operation(arg_subparsers,"validate",help="Check database integrity")
    add_standard_args(op_def)
    op_def.add_argument("what", choices=['features','layers','datasources','table_sets'], help="Type of data to check")
    op_def.add_argument("names", type=unicode, nargs="?", default="*",                    help="Item(s) to validate")
    ...
    def operation_validate(self):
        ...
        if   self.args.what == 'features':    self.validate_features(   db,self.args.names)
        elif self.args.what == 'layers':      self.validate_layers(     db,self.args.names)
        elif self.args.what == 'datasources': self.validate_datasources(db,self.args.names)
        elif self.args.what == 'table_sets':  self.validate_table_sets( db,self.args.names)
        else:
            raise Exception('Bad choice: '+self.args.what)  # Should never happen
    ...
    def validate_features(self,db,name_spec):
        ...
        for rec in db.dd.featureTypeRecs(datasource_spec,name_spec,sort=True,warn_if_no_match=True):
            
            for err_msg in rec.validate(enum_names,unit_defs):
                rows.append( { 'feature': rec, 
                               'error':   err_msg } )
    ...
    op_def = define_operation(arg_subparsers,"initialise",help="Initialise master database for replication")
    op_def.add_argument("sync_dir", type=unicode,        help="Directory to store update packages in")
    op_def.add_argument("sync_url", type=unicode,        help="URL of master myWorld server")
    ...
    def operation_initialise(self):
        ...
        rep_engine.initialiseDatabase(self.args.sync_dir, self.args.sync_url, self.args.force)
    ...
    op_def = define_operation(arg_subparsers,"extract",help="Create a SQLite extract")
    op_def.add_argument("output_db",      type=unicode,                            help="File name for SQLite database (suffix must be .db)")
    op_def.add_argument("description",    type=unicode,                            help="Description for metadata file")
    op_def.add_argument("region",         type=unicode, nargs="?", default='full', help="Region defining extract extent")
    op_def.add_argument("table_set",      type=unicode, nargs="?", default='full', help="Table set defining extract content")
    op_def.add_argument("name",           type=unicode, nargs="?",                 help="Name for extract (default: <region>_<table_set>)")
    op_def.add_argument("--zipped",       action='store_true',                     help="Zip files for download")
    op_def.add_argument("--overwrite",    action='store_true',                     help="If database files already exist, overwrite them")
    add_standard_args(op_def)
    ...
    def operation_extract(self):
        ...
        if region == 'full': region = None

        table_set = self.args.table_set
        if table_set == 'full': table_set = None
        ...
        master_db_connect_info = { 'host': self.db_server.host,
        ...
        extract_db  = rep_engine.extract(self.args.output_db,
                                         extract_name,
                                         region,
                                         table_set,
    ...
    op_def = define_operation(arg_subparsers,"export",help="Export master changes to the sync directory")
    op_def.add_argument("extract_type",       type=unicode, default="*", nargs="?", help="Extract type to export changes for")
    op_def.add_argument("--records_per_file", type=int, default=2000, metavar='N',  help="Start new output file after N records") # NativeApp prefers small files
    op_def.add_argument("--include_code", action='store_true',                      help="Include client code in sync package")
    add_standard_args(op_def)
    ...
    def operation_export(self):
        ...
        # Export the updates
        if db_type == 'master':
            rep_engine.exportChanges(self.args.extract_type,
    ...
    op_def = define_operation(arg_subparsers,"update",help="Apply master updates to a SQLite extract")
    add_standard_args(op_def)
    ...
    op_def = define_operation(arg_subparsers,"convert",help="Convert a database to SQLite format")
    op_def.add_argument("output_db",   type=unicode,        help="File name for SQLite database (suffix must be .db)")
    op_def.add_argument("--overwrite", action='store_true', help="If file already exists, overwrite it")
    add_standard_args(op_def)
    ...
    def operation_convert(self):
        ...
        from myworldapp.core.server.replication.myw_extract_filter import MywExtractFilter    # ENH: Avod need to import this
        from myworldapp.core.server.database.myw_upgrade_manager   import MywUpgradeManager
        ...
        if db_version != expected_version:
            msg = "Database schema does not match expected version: Expected {}: Got {}".format(expected_version,db_version)
            raise MywError(msg)
        ...
        engine.extractDatabase(self.args.output_db,
                               MywExtractFilter('extract'))
    ...
    op_def = define_operation(arg_subparsers,"package",help="Package a SQLite database for deployment")
    op_def.add_argument("description", type=unicode,        help="Description of database content")
    op_def.add_argument("--zipped",    action='store_true', help="Zip files for download")
    ...
    op_def = define_operation(arg_subparsers,"run",help="Run a script")
    op_def.add_argument("file", type=unicode, nargs='?',           help="File to run")
    op_def.add_argument("script_args",        nargs='*',           help="Arguments to pass to script")
    op_def.add_argument("--command","-c",     action='store_true', help="Interpret args as a python command")
    op_def.add_argument("--sql","-s",         action='store_true', help="Interpret args as a sql command")
    op_def.add_argument("--commit",           action='store_true', help="Perform a commit after script completes")
    add_standard_args(op_def)
    ...
    def operation_run(self):
        ...
        # Case: Single command
            ...
            cmd = u" ".join( [self.args.file] + self.args.script_args )
            if self.args.sql:
        ...
        # Case: Run file
            ...
            if self.args.file.endswith('.sql'):
                ...
                globs = {'db'      : db,
                         '__file__': os.path.abspath(self.args.file) }
                execfile(self.args.file,globs,globs) # Globs args enable ref of local classes in files
    ...
    def run_sql_statement(self,db,sql):
        ...
        to_string = lambda x: "<buffer>" if isinstance(x,buffer) else unicode(x)
        ...
        if res.returns_rows:
            print "  ", u",".join( map( to_string,res.keys() ) )
            for row in res:
                print "  ", u",".join( map( to_string,row ) )
    ...
    op_def = define_operation(arg_subparsers,"backup",help="Copy database to disk archive")
    op_def.add_argument("file",type=unicode, help="File to create")
    add_standard_args(op_def)
    ...
    op_def = define_operation(arg_subparsers,"restore",help="Restore database from disk archive")
    op_def.add_argument("file",type=unicode, help="File to restore from")
    add_standard_args(op_def)
    ...
    op_def = define_operation(arg_subparsers,"drop_database",help="Delete database")
    add_standard_args(op_def)
